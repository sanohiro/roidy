...
export class CastSession {
  ...
  async start() {
    ...
    try { execSync(`adb forward --remove tcp:${FORWARD_PORT}`, { stdio: 'ignore' }); } catch {}
    execSync(`adb forward tcp:${FORWARD_PORT} localabstract:scrcpy`);
    ...
    this._server = spawn('adb', ['shell', serverCmd], { stdio: ['ignore', 'pipe', 'pipe'] });
  ...
  async _ensureServer() {
    try {
      const check = execSync('adb shell "ls /data/local/tmp/scrcpy-server.jar 2>&1"', {
    ...
    try {
      ...
      if (existsSync(localPath)) {
        execSync(`adb push "${localPath}" /data/local/tmp/scrcpy-server.jar`, { timeout: 10000 });
        return;
      }
    } catch {}
