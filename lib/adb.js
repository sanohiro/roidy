...
// Run adb CLI command with device serial (-s) prefix
// For code that needs execSync('adb ...') instead of adbkit
export function adbExec(args, opts = {}) {
  const serial = _deviceId ? `-s ${_deviceId} ` : '';
  return execSync(`adb ${serial}${args}`, { encoding: 'utf8', ...opts }).trim();
}
...
// Resize our virtual display (triggered by terminal resize)
// This updates overlay_display_devices and notifies other instances
export async function resizeVirtualDisplay(width, height, density) {
  ...
  const entry = state.entries.find(e => e.pid === process.pid);
  if (!entry) return;
  entry.spec = spec;
  ...
  const newId = await waitForDisplayByIndex(_overlayIndex);
  _displayId = newId;
  ...
  for (const e of state.entries) {
    if (e.pid !== process.pid) {
      try { process.kill(e.pid, 'SIGUSR1'); } catch {}
    }
  }
}
...
// Check if app rotated the virtual display
export async function isDisplayRotated(displayId) {
  const output = await shell('dumpsys display');
  // Grab the section for our display
  const sectionRe = new RegExp(`displayId ${displayId}"[\\s\\S]*?(?=Display Id=|$)`);
  const section = output.match(sectionRe);
  if (!section) return false;
  const rotRe = /mOverrideDisplayInfo=DisplayInfo\{[\s\S]*?rotation (\d)/;
  const m = section[0].match(rotRe);
  const rotation = m ? parseInt(m[1]) : -1;
  return rotation % 2 !== 0;
}
