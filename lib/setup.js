...
import { adbExec, setDeviceId } from './adb.js';
...
function adb(cmd) {
  return adbExec(`shell "${cmd}"`, { timeout: 15000 });
}

function adbInstall(apkPath) {
  return adbExec(`install "${apkPath}"`, { timeout: 60000 });
}
...
function getInstalledApps() {
  return new Set(adb('pm list packages').split('\n').map(l => l.replace('package:', '').trim()).filter(Boolean));
}

async function installApp(app) {
  const installed = adb(`pm list packages ${app.pkg}`);
...
export async function runSetup(host, port, opts = {}) {
  ...
  setDeviceId(serial);
  try {
    execSync(`adb connect ${serial}`, { encoding: 'utf8', timeout: 10000 });
  ...
  // GApps detection — check for Google Play Services (persists even after disabling setup wizard)
  const hasGApps = adb('pm list packages com.google.android.gms')
    .includes('com.google.android.gms');
  if (hasGApps) {
    console.log('Google Play detected. The following settings are strongly recommended.\n');
    // Skip setup wizard
    const provisioned = adb('settings get global device_provisioned');
    const wizardStatus = provisioned === '1' ? 'skipped' : 'not completed';
    const doSkip = opts.skipWizard ??
      (await prompt(`Skip setup wizard? Freezes the screen in headless mode (y/N) (current: ${wizardStatus})`, 'N')).toLowerCase() === 'y';
    if (doSkip && provisioned !== '1') {
      adb('su 0 settings put global device_provisioned 1');
      adb('su 0 settings put secure user_setup_complete 1');
      try { adb('su 0 pm disable com.google.android.setupwizard'); } catch {}
      console.log('Setup wizard skipped');
    }

    // Disable Play Protect — blocks sideloaded apps from F-Droid etc.
    const verifier = adb('settings get global package_verifier_enable');
    const protectStatus = verifier === '0' ? 'disabled' : 'enabled';
    const doDisable = opts.disablePlayProtect ??
      (await prompt(`Disable Play Protect? Blocks sideloaded apps like F-Droid (y/N) (current: ${protectStatus})`, 'N')).toLowerCase() === 'y';
    if (doDisable && verifier !== '0') {
      adb('settings put global package_verifier_enable 0');
      adb('settings put global verifier_verify_adb_installs 0');
      console.log('Play Protect disabled');
    }
  }
  ...
  // Get valid timezones from device
  const validTz = new Set(
    adb('toybox strings /system/usr/share/zoneinfo/tzdata')
  ...
  if (!tz) {
    while (true) {
      tz = await prompt(`Timezone, e.g. Asia/Tokyo, America/New_York (current: ${currentTz})`, '');
      if (!tz || validTz.has(tz)) break;
      console.log(`"${tz}" is not a valid timezone. Try again.`);
    }
  }

  // Locale
  const locale = opts.locale ?? await prompt(`Locale, e.g. ja-JP, en-US, zh-CN (current: ${currentLocale})`, '');
  ...
  if (!opts.noInstall) {
    ...
    if (launcher) {
      ...
      if (ok) {
        if (launcher.permissions) {
          for (const perm of launcher.permissions) {
            try { adb(`pm grant ${launcher.pkg} ${perm}`); } catch {}
          }
        }
        setHomeActivity = launcher.homeActivity;
        setHomeName = launcher.name;
        adb(`cmd package set-home-activity ${setHomeActivity}`);
        try { adb(`cmd role add-role-holder android.app.role.HOME ${launcher.pkg}`); } catch {}
  ...
  if (needsReboot) {
    ...
    if (doRestart) {
      ...
      for (let i = 0; i < 30; i++) {
        try {
          const result = adb('getprop sys.boot_completed');
          if (result === '1') break;
        } catch {}
        execSync('sleep 2');
      }
      ...
      if (homeToApply) {
        for (let i = 0; i < 15; i++) {
          try {
            const out = adb(`cmd package set-home-activity ${homeToApply}`);
