...
export async function runSetup(host, port, opts = {}) {
  ...
  if (hasGApps) {
    ...
    const doSkip = opts.skipWizard ??
      (await prompt(`Skip setup wizard? (y/N) (current: ${wizardStatus})`, 'N')).toLowerCase() === 'y';
    ...
    const doDisable = opts.disablePlayProtect ??
      (await prompt(`Disable Play Protect? Needed for F-Droid installs (y/N) (current: ${protectStatus})`, 'N')).toLowerCase() === 'y';
  ...
  // Timezone
  const tz = opts.timezone ?? await prompt(`Timezone, e.g. Asia/Tokyo, America/New_York (current: ${currentTz})`, '');
  if (tz && !validTz.has(tz)) {
    console.log(`Warning: "${tz}" is not a known timezone — setting anyway`);
  }
